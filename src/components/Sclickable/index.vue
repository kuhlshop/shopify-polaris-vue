...
type MaybeAllValuesShorthandProperty<T extends string> =
  | T
  | `${T} ${T}`
  | `${T} ${T} ${T}`
  | `${T} ${T} ${T} ${T}`;
...
type MaybeResponsive<T extends string> = T | `@container${string}`;
...
type DisplayValue = "auto" | "none";

type OverflowValue = "visible" | "hidden";

type CommandValue = "--auto" | "--show" | "--hide" | "--toggle";
...
interface Props {
  ...
  commandFor?: string;
  disabled?: boolean;
  display?: MaybeResponsive<DisplayValue>;
  download?: string;
  href?: string;
